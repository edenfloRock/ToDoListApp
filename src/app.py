...
def main(page: Page):
    ...
    def FinalizeUpdate(e):        
        ...
        db = db_instance.ConnectToDatabase()
        db_instance.UpdateDatabase(
            (
                #first we need to get the data we want to change with...
                form.content.controls[0].value,
                # second, we insert the data to check against (the value that this query should look for)
                e.controls[0].content.controls[0].controls[0].value,
             )
        )
    ...
    def LoadTasks():
        ...
        #_main_column_.controls.clear()
        for x in _main_column_.controls[::-1]:
            print("type:" + str(type(x)))
            if isinstance(x, CreateTask):
                _main_column_.controls.remove(x)
